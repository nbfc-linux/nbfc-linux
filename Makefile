...
XML_LDLIBS = $(shell pkg-config --libs libxml-2.0)

ifeq ($(BUILD), debug)
	CFLAGS   = $(LUA_CFLAGS) $(XML_CFLAGS) -Og -g
else
	CPPFLAGS = -DNDEBUG
	CFLAGS   = $(LUA_CFLAGS) $(XML_CFLAGS) -Wall -Os -flto -fno-unwind-tables -fno-asynchronous-unwind-tables -fomit-frame-pointer
	LDFLAGS  = -s
endif

LDLIBS_CLIENT = -lm -lcurl -lcrypto $(LUA_LDLIBS) $(XML_LDLIBS) -ldl
LDLIBS_SERVICE = -lm $(LUA_LDLIBS) -ldl
LDLIBS_EC_PROBE = $(LUA_LDLIBS) -ldl
...
src/nbfc: \
	...
	src/client/cmd_acpi_dump.c \
	src/client/cmd_config.c \
	src/client/cmd_misc.c \
	src/client/cmd_rate_config.c \
	...
	src/client/cmd_support.c \
	src/client/cmd_test_config.c \
	src/client/cmd_update.c \
	src/client/cmd_warranty.c \
	src/client/cmd_xml2json.c \
